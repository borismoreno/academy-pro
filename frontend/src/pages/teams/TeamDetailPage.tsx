    ...
    return (
      <div className="flex flex-col gap-4">
        <Button
          ...
          className="self-start gap-1.5 min-h-[44px] -ml-2"
        >
          <ChevronLeft size={16} />
          Equipos
        </Button>
    ...
    <>
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <SectionCard>
            <TeamCoachesList coaches={team.coaches} teamId={team.id} role={role} />
          </SectionCard>
          <SectionCard>
            <TeamSchedulesList schedules={team.schedules} teamId={team.id} role={role} />
          </SectionCard>
        </div>
              ...
              <div className="flex flex-col gap-1">
                {teamPlayers.map((player) => (
                  <button
                    ...
                    className="flex items-center gap-3 min-h-[44px] px-3 rounded-xl hover:bg-surface-highest transition-colors text-left -mx-3"
                  >
                    ...
                    {player.position && (
                      <span className="flex-shrink-0 font-body text-[0.6875rem] uppercase tracking-[0.05em] bg-surface-highest text-on-surface-variant rounded-full px-2.5 py-0.5">
                        {player.position}
                      </span>
                    )}
                  </button>
