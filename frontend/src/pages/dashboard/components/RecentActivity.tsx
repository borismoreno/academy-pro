...
import type { EvaluationResponse, SessionListResponse, EvaluationScore } from '@/services/dashboard.service';
...
function SkeletonRow({ index }: { index: number }) {
  return (
    ...
    >
      <div className="w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-surface-highest flex-shrink-0" />
      ...
      <div className="hidden lg:block h-6 w-14 bg-surface-highest rounded-full flex-shrink-0" />
    </div>
  );
}
...
export default function RecentActivity({ recentEvaluations, recentSessions, isLoading }: RecentActivityProps) {
        ...
        <>
          <div className="space-y-2">
            {allItems.map((item, index) => {
              ...
              const hiddenOnMobile = index >= MOBILE_LIMIT && !showAll ? 'hidden lg:flex' : 'flex';
              ...
              if (item.type === 'evaluation') {
                ...
                return (
                  ...
                  >
                    {/* Icon */}
                    <div className="w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-surface-highest flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-body text-[0.875rem] text-on-surface truncate">
                        <span className="font-medium">{item.data.player.fullName}</span>
                        {' fue evaluado por '}
                      ...
                      <p className="font-body text-[0.6875rem] uppercase tracking-[0.05em] text-on-surface-variant mt-0.5">
                        ...
                        <span className="lg:hidden">{getCompactRelativeTime(item.date)}</span>
                        <span className="hidden lg:inline">{getRelativeTime(item.date)}</span>
                      </p>
                    </div>

                    {/* Chip — hidden on mobile */}
                    {avg !== null && (
                      <span className="hidden lg:flex flex-shrink-0 px-3 py-1 rounded-full bg-primary-container text-on-primary font-body text-[0.6875rem] font-medium">
                        {avg}
                      </span>
                    )}
                  </div>
              ...
              // session
              const sessionDateStr = new Date(item.data.sessionDate).toLocaleDateString('es-EC');
              return (
                <div
                  key={item.data.id}
                  className={`${hiddenOnMobile} items-center gap-3 lg:gap-4 px-3 lg:px-4 py-2 lg:py-3 rounded-xl ${rowBg}`}
                >
                  ...
                  {/* Chip — hidden on mobile */}
                  <span className="hidden lg:flex flex-shrink-0 px-3 py-1 rounded-full bg-secondary-container text-on-surface font-body text-[0.6875rem] font-medium whitespace-nowrap">
          ...
          {hasMore && (
            <button
              ...
              className="lg:hidden w-full min-h-[44px] font-body text-[0.875rem] text-on-surface-variant hover:text-primary transition-colors"
            >
