...
import { createSession, bulkUpdateRecords } from '@/services/attendance.service';
import type { CreateSessionData, BulkUpdateData } from '@/services/attendance.service';
  ...
  return (
    <div className="px-6 pb-4 flex flex-col gap-4">
      ...
      <button
        type="button"
        onClick={onFinish}
        className="w-full h-12 rounded-xl font-body font-semibold text-[0.875rem] bg-gradient-to-br from-primary to-secondary text-on-primary transition-opacity hover:opacity-90 cursor-pointer"
      >
        Finalizar
      </button>
    </div>
...
export default function SessionFormSheet({ open, onOpenChange }: SessionFormSheetProps) {
  ...
  const content =
    step === 'form' ? (
      ...
      <AttendanceStep key={contentKey} session={createdSession!} onFinish={handleFinish} />
    );
